...
def qa_pair_generator():
    with st.expander("💡 问答对生成器（选用工具）"):
        st.write("注：如果报错，很可能是由于敏感词问题")
        
        num_group = st.text_input("**问答对组数**", placeholder="默认：5组（最大组数为12）").strip()
        context = st.text_area("**背景信息**", placeholder="默认：无，例如：这个电话客服是基于线下实体店的，是线下类似于剧本杀，棋牌游戏的服务行业").strip()
        question = st.text_input("**期望问题**:red[*]", placeholder="例如：客户的电话投诉").strip()
        answer = st.text_input("**期望回答:red[*]**", placeholder="例如：标准而礼貌的客服回复").strip()
